 ...
 */
static size_t alignment_up(size_t req_size) {
 ...
 * 	in search of free block that can fit specified size
 *
 * Returns NULL when no block can fit the size
 *
 * scan_headers_size doesn't guarantee that 
 * 	the block can be split unless the inputted
 * 	size already accounts for the header_size
 */
static Header *scan_headers_size(size_t target_size) {
 ...
 */
static Header *scan_headers_address(void *init_ptr) {
	Header *cur_header = head;
 ...
 */
static Header *merge_next_block(Header *header1) {
 ...
 *
 */
static Header *split_block(Header *cur_header, size_t size) {
